...
const chestExercises = {
  weightLoss: [
    ...
    { name: "Dumbbell Pullovers", sets: 3, reps: "15-20", description: "Chest and lat stretch", safety: "Control the weight" },
  ],
  muscleBuild: [
    ...
    { name: "Chest Dips", sets: 3, reps: "8-12", description: "Compound chest exercise", safety: "Proper form" },
  ]
};
...
const backExercises = {
  weightLoss: [
    ...
    { name: "Lat Pulldowns", sets: 3, reps: "15-20", description: "Wide grip pull", safety: "Control the bar" },
  ],
  muscleBuild: [
    ...
    { name: "Close-Grip Lat Pulldowns", sets: 3, reps: "8-12", description: "Lat isolation", safety: "Control the bar" },
  ]
};
...
const legExercises = {
  weightLoss: [
    ...
    { name: "Bulgarian Split Squats", sets: 3, reps: "15-20", description: "Unilateral strength", safety: "Balance" },
  ],
  muscleBuild: [
    ...
    { name: "Lunges", sets: 3, reps: "10-12", description: "Unilateral strength", safety: "Step control" },
  ]
};
...
const shoulderExercises = {
  weightLoss: [
    ...
    { name: "Lateral Raises", sets: 3, reps: "15-20", description: "Side delt isolation", safety: "Control the weight" },
  ],
  muscleBuild: [
    ...
    { name: "Dumbbell Shrugs", sets: 3, reps: "12-15", description: "Trap activation", safety: "Control the weight" },
  ]
};
...
const armExercises = {
  weightLoss: [
    ...
    { name: "Concentration Curls", sets: 3, reps: "15-20", description: "Bicep isolation", safety: "Control the weight" },
  ],
  muscleBuild: [
    ...
    { name: "Zottman Curls", sets: 3, reps: "10-12", description: "Bicep and forearm", safety: "Control the weight" },
  ]
};
...
const coreExercises = {
  weightLoss: [
    ...
    { name: "Leg Raises", sets: 3, reps: "15-20", description: "Lower abs", safety: "Control the movement" },
  ],
  muscleBuild: [
    ...
    { name: "Russian Twists with Medicine Ball", sets: 3, reps: "15-20", description: "Rotational move", safety: "Control the ball" },
  ]
};
...
const WorkoutChallenge = () => {
  ...
  const confirmCompletion = () => {
    if (currentDayIndex !== null) {
      setWorkoutPlan(prev => {
        const newPlan = [...prev];
        newPlan[currentDayIndex] = { ...newPlan[currentDayIndex], completed: true };
    ...
    <CardContent className="p-0 bg-black">
      <div className="p-4 flex flex-col gap-4">
        <div className="flex items-center justify-between bg-black rounded-lg p-3 border border-gray-800">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-white">{workoutType === 'weight-loss' ? 'Weight Loss' : 'Muscle Gain'} Challenge</span>
          </div>
          <div className="flex items-center space-x-2">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 order-2 md:order-1">
            <Accordion type="single" collapsible className="bg-black rounded-lg border border-gray-800">
              {workoutPlan.map((day, index) => (
                <AccordionItem key={index} value={`day-${day.day}`} className="border-b border-gray-800 last:border-0">
                  <AccordionTrigger className="px-4 py-3 hover:bg-gray-900 text-white">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="flex items-center">
                        <span className={`mr-2 ${day.completed ? 'line-through text-gray-500' : 'text-white'}`}>
                          Day {day.day}
                        </span>
                  ...
                  <AccordionContent className="px-4 py-2 bg-gray-900">
                    {day.exercises.length > 0 ? (
                      <div className="space-y-3 py-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                        {day.exercises.map((exercise, exerciseIndex) => (
                          <div key={exerciseIndex} className="bg-gray-800 p-3 rounded-lg border border-gray-700">
                            <h4 className="font-medium text-white">{exercise.name}</h4>
                            <div className="flex flex-wrap gap-2 mt-1.5">
                              <span className="text-xs bg-gray-900 px-2 py-0.5 rounded text-white">Sets: {exercise.sets}</span>
                              <span className="text-xs bg-gray-900 px-2 py-0.5 rounded text-white">Reps: {exercise.reps}</span>
                            </div>
      ...
      <AlertDialog open={showDialog} onOpenChange={setShowDialog}>
        <AlertDialogContent className="bg-black border border-gray-800">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">Change Workout Type?</AlertDialogTitle>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmTypeChange} className="bg-gym-accent hover:bg-gym-accent/90 text-black">Continue</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>

      <AlertDialog open={completionDialog} onOpenChange={setCompletionDialog}>
        <AlertDialogContent className="bg-black border border-gray-800">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">Mark as Completed?</AlertDialogTitle>
            <AlertDialogDescription className="text-gray-400">
              Are you sure you completed the exercises for this day?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={cancelCompletion} className="bg-gray-800 hover:bg-gray-700 text-white border-0">Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmCompletion} className="bg-gym-accent hover:bg-gym-accent/90 text-black">Yes, I did</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </CardContent>
